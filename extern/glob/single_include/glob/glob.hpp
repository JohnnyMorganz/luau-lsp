    ...
    if (fs::exists(current_directory))
    {
        try
        {
            for (auto& entry : fs::directory_iterator(
                     current_directory, fs::directory_options::follow_directory_symlink | fs::directory_options::skip_permission_denied))
            {
...
// directory.
static inline std::vector<fs::path> glob2(const fs::path& dirname, [[maybe_unused]] const std::string& pattern, bool dironly)
{
    // std::cout << "In glob2\n";
    std::vector<fs::path> result;
...
static inline std::vector<fs::path> glob(const std::string& pathname, bool recursive = false, bool dironly = false)
{
    std::vector<fs::path> result;
...
static inline std::vector<std::filesystem::path> glob(const std::vector<std::string>& pathnames)
{
...
static inline std::vector<std::filesystem::path> rglob(const std::vector<std::string>& pathnames)
{
    std::vector<std::filesystem::path> result;
    for (auto& pathname : pathnames)
    {
...
static inline std::vector<std::filesystem::path> glob(const std::initializer_list<std::string>& pathnames)
{
    return glob(std::vector<std::string>(pathnames));
}

static inline std::vector<std::filesystem::path> rglob(const std::initializer_list<std::string>& pathnames)
{
