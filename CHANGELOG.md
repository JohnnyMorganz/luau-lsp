...
- A local bound to a module that exports only types now shows an inlay hint listing those types, such as `: { type User, type Id }`, in place of `: {}`. Each type name shows its documentation and links to its definition, and the hint cannot be inserted, since the empty table it would write is not what the module offers
...
- Sourcemap is no longer autogenerated from Studio Plugin info when `sourcemap.enabled` is set to `false`
...
- In VSCode, added `luau-lsp.server.baseLuaurc` to configure a path to the baseline `.luaurc` file to load across all workspaces. External editors can already access this by using the `--base-luaurc` command line flag. ([#1262](https://github.com/JohnnyMorganz/luau-lsp/issues/1262))
